...
The checklist has other real, still-open findings (the check-then-act race, missing controller-level test coverage) that could ground an SBI example just as legitimately. This one was chosen because it has the richest evidence trail — an exact line, a measured before/after (48/50 failing, 50/50 passing), and a real follow-up action already taken — which is what makes the Situation and Impact sections concrete instead of hand-wavy. A vaguer finding would force this example to fall back on hedging language ("this could have caused issues"), which defeats the point of practicing SBI on real material. Worth naming directly: this is the same reasoning that made [the hiring question set](hiring-question-set.md)'s Question 5 converge on this exact same finding independently — not a coincidence, since it's genuinely the richest-evidence finding in the whole epic, used now by four sibling artifacts (the postmortem, the checklist itself, Question 5, and this document) for the same underlying reason.
